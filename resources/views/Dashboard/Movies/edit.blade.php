                    ...
                    <div class="card mb-4">
                      ...
                      @endif
                        ...
                        <div class="card-body">
                            ...
                            @endif
                            <form action="{{ route("dashboard.movies.update", ['movie' => $movie->id]) }}" class="needs-validation" method="POST" enctype="multipart/form-data" novalidate>
                                @csrf
                                <div class="input-group mb-3">
                                  <span class="input-group-text">عنوان اصلی فیلم</span>
                                  <input type="text" value="{{ old('title[0]', isset($movie) ? $movie->title[0] : "") }}" aria-label="title" id="bs-validation-name" placeholder="عنوان اصلی" name="title[]" class="form-control" required>
                                  <div class="invalid-feedback">در این قسمت فقط عنوان فیلم را بنویسید</div>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">عنوان انگلیسی فیلم</span>
                                  <input type="text" value="{{ old('title[1]', isset($movie) ? $movie->title[1] : "") }}" aria-label="title" id="bs-validation-name" placeholder="عنوان انگلیسی فیلم" name="title[]" class="form-control" required>
                                  <div class="invalid-feedback">در این قسمت فقط عنوان فیلم را بنویسید</div>
                                </div>
                                <div class="input-group mb-3">
                                ...
                                <div class="input-group mb-3">
                                  ...
                                  <input type="text" value="{{ old('imdb', isset($movie) ? $movie->imdb: "") }}" aria-label="imdb" id="bs-validation-name" placeholder="رتبه IMDB" name="imdb" class="form-control" required>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">سال ساخت</span>
                                  <input type="text" value="{{ old('creation_year', isset($movie) ? $movie->creation_year: "") }}" aria-label="imdb" id="bs-validation-name" placeholder="سال ساخت" name="creation_year" class="form-control" required>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">درجه سنی</span>
                                  <input type="text" value="{{ old('age', isset($movie) ? $movie->age: "") }}" aria-label="age" id="bs-validation-name" placeholder="درجه سنی" name="age" class="form-control" required>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">کشور سازنده</span>
                                  <input type="text" value="{{ old('country', isset($movie) ? $movie->country: "") }}" aria-label="country" id="bs-validation-name" placeholder="کشور سازنده" name="country" class="form-control" required>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">مدت زمان</span>
                                  <input type="text" value="{{ old('time', isset($movie) ? $movie->time: "") }}" aria-label="time" id="bs-validation-name" placeholder="کشور سازنده" name="time" class="form-control" required>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">داستان فیلم</span>
                                  <textarea class="form-control" name="story">{{ old('story', isset($movie) ? $movie->story: "") }}</textarea>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">درباره فیلم</span>
                                  <textarea class="form-control" name="about">{{ old('about', isset($movie) ? $movie->about: "") }}</textarea>
                                </div>
                                <div class="input-group mb-3">
                                ...
                                <div class="input-group mb-3">
                                  ...
                                  <select name="genres[]" class="select2 form-select form-select-lg" data-allow-clear="true" multiple>
                                    @foreach ($genres as $genre)
                                      <option value="{{ $genre->id }}" @if (in_array($genre->id, $movieGenres)) selected @endif>{{  $genre->title }}</option>
                                    @endforeach
                                  </select>
                                </div>
                                <div class="input-group mb-3">
                                  ...
                                  <select name="lists[]" class="select2 form-select form-select-lg" data-allow-clear="true" multiple>
                                    @foreach ($lists as $list)
                                      <option value="{{ $list->id }}" @if (in_array($list->id, $movieLists)) selected @endif>{{  $list->title }}</option>
                                    @endforeach
                                  </select>
                                </div>
                                <div class="input-group mb-3">
                                ...
                                <div class="input-group mb-3">
                                  ...
                                  <select id="type" class="selectpicker w-100" data-style="btn-default" name="type">
                                    <option value="movie" @if ($movie->type == "movie") selected @endif>فیلم</option>
                                    <option value="series" @if ($movie->type == "series") selected @endif>سریال</option>
                                    <option value="animation" @if ($movie->type == "animation") selected @endif>انیمیشن</option>
                                    <option value="anime" @if ($movie->type == "anime") selected @endif>انیمه</option>
                                  </select>
                                </div>
...
$(document).ready(function() {
  ...
  let selectedDirectors = @json($selectedDirectors);
  let selectedCollection = @json($selectedCollection);
  ...
  if (selectedDirectors.length > 0) {
    selectedDirectors.forEach(director => {
        let option = new Option(director.text, director.text, true, true);
        $directorSelect.append(option);
    });
    $directorSelect.trigger('change');
  }
  let selectedActors = @json($selectedActors);
  $actorSelect.select2({
  ...
  if (selectedActors.length > 0) {
    selectedActors.forEach(actor => {
        let option = new Option(actor.text, actor.id, true, true);
        $actorSelect.append(option);
    });
    $actorSelect.trigger('change');
  }
  ...
  if (selectedCollection.length > 0) {
    selectedCollection.forEach(collection => {
        let option = new Option(collection.text, collection.id, true, true);
