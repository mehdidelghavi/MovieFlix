            ...
            <div class="row d-flex justify-content-center">
                ...
                <div class="col-lg-6 col-md-12 col-sm-12">
                    ...
                    <div class="card mb-4">
                      ...
                      @endif
                        <h5 class="card-header heading-color">ویرایش تعرفه</h5>
                        <div class="card-body">
                            @if ($errors->any())
                            ...
                            @endif
                            <form action="{{ route("dashboard.plans.update", ['plan' => $plan->id]) }}" class="needs-validation" method="POST" enctype="multipart/form-data" novalidate>
                                @csrf
                                <div class="input-group mb-3">
                                  <span class="input-group-text">عنوان تعرفه</span>
                                  <input type="text" value="{{ old('title', $plan->title) }}" aria-label="First name" id="bs-validation-name" placeholder="تعرفه طلایی" name="title" class="form-control" required>
                                  <div class="invalid-feedback">لطفا عنوان تعرفه را وارد کنید</div>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">مدت زمان</span>
                                  <input type="number" value="{{ old('duration', $plan->duration) }}" required id="phone-number-mask" name="duration" class="form-control" placeholder="عدد به روز">
                                  <div class="invalid-feedback">لطفا مدت زمان وارد کنید</div>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">قیمت (تومان)</span>
                                  <input type="text" value="{{ old('price', $plan->price) }}" required name="price" class="form-control numeral-mask text-start" placeholder="30,000">
                                  <div class="invalid-feedback">لطفا قیمت وارد کنید</div>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">تخفیف (%)</span>
                                  <input type="number" value="{{ old('discount', $plan->discount) }}" required name="discount" class="form-control">
                                  <div class="invalid-feedback">لطفا قیمت وارد کنید</div>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">درباره تعرفه</span>
                                  <textarea class="form-control" name="about" aria-label="With textarea" placeholder="این تعرفه شامل فقط تماشای آنلاین میباشد">{{ old("about", $plan->about) }}</textarea>
                                </div>
