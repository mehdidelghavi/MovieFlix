    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 breadcrumb-wrapper mb-4">
                <span class="text-muted fw-light">کالکشن ها /</span> لیست کالکشن ها
            </h4>

            <div class="row d-flex justify-content-center">
            <div class="card">
                <h5 class="card-header heading-color">لیست کالکشن ها</h5>
                @if(session('success'))
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                  <form action="{{ route("dashboard.collections.multiDelete") }}" method="POST" class="MProccessForm">
                    @csrf
                    <div class="d-flex w-100 justify-content-center align-items-center">
                        <div style="width: 163px">
                            <select id="defaultSelect" class="form-select MProccess">
                                <option disabled selected>انجام گروهی</option>
                                <option value="1">حذف گروهی کالکشن ها</option>
                            </select>
                        </div>
                    </div>
...
<script>
    $(document).ready( function (){
        ...
        if (dt_select_table.length) {
            ...
            ajax: "{{ route("dashboard.collections") }}",
            columns: [
                { data: 'id', name: 'id' },
                { data: 'name', name: 'name' },
                { data: 'thumbnail', name: 'avatar' },
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        return `<input type="checkbox" class="dt-checkboxes form-check-input mt-0 align-middle" name="collections[]" value="${row.id}">`;
                    },
                    checkboxes: {
                        selectRow: true,
