    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 breadcrumb-wrapper mb-4">
                <span class="text-muted fw-light">ژانر ها /</span> لیست ژانر ها
            </h4>

            <div class="row d-flex justify-content-center">
            <div class="card">
                <h5 class="card-header heading-color">لیست ژانر ها</h5>
                @if(session('success'))
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                  <form action="{{ route("dashboard.genres.multiDelete") }}" method="POST" class="MProccessForm">
                    @csrf
                    <div class="d-flex w-100 justify-content-center align-items-center">
                        <div style="width: 163px">
                            <select id="defaultSelect" class="form-select MProccess">
                                <option disabled selected>انجام گروهی</option>
                                <option value="1">حذف گروهی ژانر ها</option>
                            </select>
                        </div>
                    </div>
...
<script>
    $(document).ready( function (){
        ...
        if (dt_select_table.length) {
            ...
            ajax: "{{ route("dashboard.genres") }}",
            columns: [
                { data: 'id', name: 'id' },
                { data: 'title', name: 'title' },
                { data: 'genre_movie', name: 'genre_movie' },
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        return `<input type="checkbox" class="dt-checkboxes form-check-input mt-0 align-middle" name="genres[]" value="${row.id}">`;
                    },
                    checkboxes: {
                        selectRow: true,
