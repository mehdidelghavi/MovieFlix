    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 breadcrumb-wrapper mb-4">
                <span class="text-muted fw-light">خبرنامه /</span> اعضای خبرنامه
            </h4>

            <div class="row d-flex justify-content-center">
            <div class="card">
                <h5 class="card-header heading-color">اعضای خبرنامه</h5>
                @if(session('success'))
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                  <form action="{{ route("dashboard.newsletter.multiDelete") }}" method="POST" class="MProccessForm">
                    @csrf
                    <div class="d-flex w-100 justify-content-center align-items-center">
                        <div style="width: 163px">
                            <select id="defaultSelect" class="form-select MProccess">
                                <option disabled selected>انجام گروهی</option>
                                <option value="1">حذف گروهی اعضا</option>
                            </select>
                        </div>
                    </div>
...
<script>
    $(document).ready( function (){
        ...
        if (dt_select_table.length) {
            ...
            ajax: "{{ route("dashboard.newsletter") }}",
            columns: [
                { data: 'id', name: 'id' },
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        return `<input type="checkbox" class="dt-checkboxes form-check-input mt-0 align-middle" name="ids[]" value="${row.id}">`;
                    },
                    checkboxes: {
                        selectRow: true,
