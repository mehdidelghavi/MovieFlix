    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 breadcrumb-wrapper mb-4">
                <span class="text-muted fw-light">نقش ها /</span> ویرایش نقش 
            </h4>
                    ...
                    <div class="card mb-4">
                      ...
                      @endif
                        ...
                        <div class="card-body">
                            ...
                            @endif
                            <form action="{{ route("dashboard.roles.update", ['role' => $role->id]) }}" class="needs-validation" method="POST" enctype="multipart/form-data" novalidate>
                                @csrf
                                <div class="input-group mb-3">
                                  <span class="input-group-text">نام نقش</span>
                                  <input type="text" value="{{ old('name', isset($role) ? $role->name : "") }}" aria-label="First name" id="bs-validation-name" placeholder="نام" name="name" class="form-control" required>
                                </div>
                                <div class="input-group mb-3">
                                  <span class="input-group-text">سطح نقش</span>
                                  <input type="text" value="{{ old('guard_name', isset($role) ? $role->guard_name : "") }}" aria-label="First name" id="bs-validation-name" placeholder="سطح" name="guard_name" class="form-control">
                                </div>
                                <div class="row row-bordered g-0 mb-3">
                                  <div class="col-12">
                                    @foreach ($permissions as $permission)
                                      <div class="form-check form-check-inline mt-3">
                                        <input class="form-check-input" type="checkbox" id="permission{{ $permission->id }}" value="{{ $permission->name }}" name="permissions[]" @if($role->hasPermissionTo($permission->name)) checked @endif>
